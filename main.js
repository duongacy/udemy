...
function createMenu1(array) {
    ...
    for (let i = 0; i < array.length; i++) {
        let item = array[i];
        let navLink =
            `<a class="nav-link show-menu-2" show=${item.id}>${item.parent}${rightIcon}</a>`

        $q("#append-menu-1").outerHTML += `<li class="nav-item">${navLink}</li>`;
...
function createMenu3(listItem, id, parent) {
    createUlMenu3(id, parent);
...
function createLiMenu2(listItem, id) {
    ...
    for (let i = 0; i < listItem.length; i++) {
        ...
        if (typeof (listItem[i]) === "object") {
            $q(`#${id} ul`).innerHTML += `<li class="nav-item">
                <a class="nav-link show-menu-3" href="#" show=${listItem[i].id}
                    ><span><i class="${listItem[i].icon} icon" ></i >${listItem[i].parent}</span>
                     ${rightIcon}
                </a>
        </li>`
            createMenu3(listItem[i].children, listItem[i].id, listItem[i].parent);
        }
    }
}
function createUlMenu3(id, parent) {
    ...
    let backMenu2Li =
        `<li class="nav-item">
            ...
            ${leftIcon}
            <i class="fab fa-connectdevelop fs-3 me-2" ></i> ${parent} </a>
        </li>`
